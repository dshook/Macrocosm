...
/// </summary>
public class PercentageOfParentFitter : UIBehaviour, ILayoutSelfController
{
  ...
  private void UpdateRect()
  {
    ...
    var newWidth = parentRectTransform.rect.width * percentage.x;
    var newHeight = parentRectTransform.rect.height * percentage.y;
    if(useMinSize){
      newWidth = Mathf.Clamp(newWidth, minSize, newWidth);
      newHeight = Mathf.Clamp(newHeight, minSize, newHeight);
    }
    if(useMaxSize){
      newWidth = Mathf.Clamp(newWidth, newWidth, maxSize);
      newHeight = Mathf.Clamp(newHeight, newHeight, maxSize);
    }
